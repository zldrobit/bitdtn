...
#include "time.h"
 ...
 /*   struct URI src_bp_endpoint_id;
	...
	while(1){
		...
		int filelength = bp_recv(bpsock, recvbuf, 1024);
		...
		if(p->type == file_dir)
		{
			...
			if((recv[0]) == 7){
                ...
                printf("the recieved file name in the metadata is %s,length is %d\n",file_name,sizeof(recv));
		...
		if(p->type == file_data)
		{
			...
			if(segment_flag == 0)
			{
				...
				printf("the recived PDU is %d,offset is %d\n",sizeof(recv),offset);
				if((offset+data_length)<meta.file_size)
				{
					...
					printf("have store the offset data %d\n",offset/data_length);
