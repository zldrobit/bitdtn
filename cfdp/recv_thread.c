		...
		if(p->type == file_dir)
		{
				...
				if(workmode == 1)
			{
				...
				if(metadata_flag == 0)
				{
					...
					printf("the file size in metadata is %d\n",meta.file_size);
	        			meta.segmentation_control = (recv_[1] >> 7);
					printf("the segment control in metadata is %d\n",meta.segmentation_control);
		...
		if((metadata_flag ==  1 && workmode == 1) || workmode == 0 )
		{
			...
			if(segment_flag == 0)
			{
				...
				if((offset+data_length)<meta.file_size)
				{
					...
					//printf("recvdata is put in buffer %d\n",offset/data_length);
					//printf("%s\n",recieve->CFDP_buffer_data[offset/data_length]);
			/*write_return = fwrite(recieve->CFDP_buffer_data[0],sizeof(char),data_length,fp);
			...
			perror("fwrite");*/
				...
				else
				{
					...
					//printf("%s\n",recieve->CFDP_buffer_data[offset/data_length]);
			/*write_return = fwrite(recieve->CFDP_buffer_data[1],sizeof(char),(meta.file_size - (meta.file_size/data_length)*data_length),fp);
			...
			else if(segment_flag == 1)
			{
				...
				memcpy(recieve->CFDP_buffer_data[0],recv_,meta.file_size);
				printf("the recieved file is %s\n",recieve->CFDP_buffer_data[0]);
				write_file(meta,fp,recieve);
				/*fp = fopen((const char*)meta.destination_file_name,"w");
					//(void)getc(fp);
    					if( fp == NULL ) {
				...
				//write_return = fwrite(a,meta.file_size,sizeof(char),fp);			
